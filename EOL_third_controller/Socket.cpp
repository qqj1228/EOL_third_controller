...
Socket_Base::Socket_Base(const bool bTCP = true) : s_(0), bTCP(bTCP) {
	Start();
	if (bTCP) {
		// Use TCP
...
Socket_Base::Socket_Base(SOCKET s, const bool bTCP = true) : s_(s), bTCP(bTCP) {
	Start();
	refCounter_ = new int(1);
};
...
SocketServer::SocketServer(int port, const bool bTCP = true, int connections = 10, TypeSocket type = BlockingSocket) {
	this->bTCP = bTCP;
...
SocketSelect::SocketSelect(Socket_Base const * const s1, Socket_Base const * const s2 = NULL, TypeSocket type = BlockingSocket) {
	FD_ZERO(&fds_);
