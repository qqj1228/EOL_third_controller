...
public:
	...
	DWORD Write(char *szBuffer, DWORD dwLength, char * szFormat, ...)
	{
		...
		_vsnprintf_s(szBuffer, CN_COMM_BUFFER_MIN_BLOCK_SIZE, dwLength, szFormat, va);
		va_end(va);
	...
	DWORD Write(wchar_t *szBuffer, DWORD dwLength, wchar_t * szFormat, ...)
	{
		va_list va;
		va_start(va, szFormat);
		_vsnwprintf_s(szBuffer, CN_COMM_BUFFER_MIN_BLOCK_SIZE, dwLength, szFormat, va);
		va_end(va);
	...
	DWORD Write(char *szBuffer, char * szFormat, ...)
	{
		va_list va;
		va_start(va, szFormat);
		vsprintf_s(szBuffer, CN_COMM_BUFFER_MIN_BLOCK_SIZE, szFormat, va);
		va_end(va);
	...
	DWORD Write(wchar_t *szBuffer, wchar_t * szFormat, ...)
	{
		va_list va;
		va_start(va, szFormat);
		vswprintf_s(szBuffer, CN_COMM_BUFFER_MIN_BLOCK_SIZE, szFormat, va);
		va_end(va);
	...
	static bool Assert(LPCTSTR szMessage)
	{
		...
		if (dwError)//! 错误代码(GetLastError())不为 0 输出错误描述  
		{
			...
			_sntprintf_s(szMsg + dwLength, CN_COMM_BUFFER_MIN_BLOCK_SIZE, 256 - _tcslen(szMsg), _T("Code:%d; "), dwError);
			dwLength = _tcslen(szMsg);
